...
"</select>"
"<div id='model_api_url_group' style='display:none'>"
...
"var modelApiUrl=document.getElementById('model_api_url').value.trim();"
"var statusEl=document.getElementById('status');"
"if(provider==='custom'){"
"if(!modelApiUrl){"
"statusEl.style.display='block';"
"statusEl.textContent='Error: Model API URL is required when provider is custom.';"
"return;"
"}"
"if(!/^https:\\/\\/(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\\.)*(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?::\\d{1,5})?(?:\\/.*)?$/.test(modelApiUrl)){"
"statusEl.style.display='block';"
"statusEl.textContent='Error: Model API URL must be a valid HTTPS URL.';"
"return;"
"}"
"}"
"var fields=['ssid','password','api_key','model','provider','model_api_url','tg_token',"
"'feishu_app_id','feishu_app_secret','proxy_host','proxy_port','proxy_type','search_key','tavily_key'];"
"var data={};"
"fields.forEach(f=>{data[f]=document.getElementById(f).value.trim()});"
"statusEl.style.display='block';"
"statusEl.textContent='Saving... Device will restart.';"
"fetch('/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)})"
".then(()=>{statusEl.textContent='Saved! Restarting...';})"
