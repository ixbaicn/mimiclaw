...
You send a message on Telegram. The ESP32-S3 picks it up over WiFi, feeds it into an agent loop — the LLM thinks, calls tools, reads memory — and sends the reply back. Supports **Anthropic (Claude)**, **OpenAI (GPT)**, and **DeepSeek** providers, plus a custom OpenAI-compatible endpoint mode, all switchable at runtime. Everything runs on a single $5 chip with all your data stored locally on flash.
...
- An **Anthropic API key** (or **OpenAI / DeepSeek API key**)
...
#define MIMI_SECRET_MODEL_PROVIDER  "anthropic"     // "anthropic" | "openai" | "deepseek" | "custom"
#define MIMI_SECRET_MODEL_API_URL   ""              // OpenAI-compatible endpoint for provider=custom
...
mimi> set_model_provider deepseek  # switch provider (anthropic|openai|deepseek|custom)
mimi> set_model gpt-4o             # change LLM model
mimi> set_model_api_url https://host/v1/chat/completions   # endpoint for provider=custom
...
- **Multi-provider** — supports Anthropic (Claude), OpenAI (GPT), DeepSeek, and custom OpenAI-compatible endpoints
